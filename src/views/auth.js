...
/* Link the legal page names inside copy that has ALREADY been escaped.
   Order matters: running esc() after inserting the <a> tags (as the sign-in
   page used to) prints the markup as literal text. */
const withLegalLinks = (escapedText) =>
  escapedText
    .replace('User Agreement', '<a href="/user-agreement">User Agreement</a>')
...
export function signInPage() {
  ...
  return shell(`
    ...
    <p class="auth-consent">${withLegalLinks(esc(p.consentNote))}</p>`);
}
  ...
  return shell(`
    ...
    <form class="auth-form" data-demo-form>
      ...
      <label class="checkbox">
        ...
        <span>${withLegalLinks(esc(p.agreementCheckbox))}</span>
      </label>
      <button class="btn btn-primary btn-block" type="submit">${esc(p.submit)}</button>
    </form>
    ${oauthBlock(p.oauth.divider)}
    <p class="auth-consent">${withLegalLinks(esc(p.consentNote))}</p>`, true);
}
